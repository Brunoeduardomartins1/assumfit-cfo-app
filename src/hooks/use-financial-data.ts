 ...
 * Subscribes to Realtime for live sync between users.
 */
export function useFinancialData(orgId: string | null): FinancialData {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (raw.transactions.length === 0) {
        setData((d) => ({ ...d, loading: false, fromDb: false }))
        return
      }
      ...
      const allMonthKeys = new Set([...monthMap.keys(), ...dreMap.keys()])
      ...
      if (latestMonth) {
        for (const tx of raw.transactions) {
          if (tx.month.startsWith(latestMonth) && Number(tx.amount) < 0) {
  ...
  // Realtime sync — re-fetch when transactions or income_statement change
  const tables = useMemo(() => ["transactions", "income_statement"], [])
  useRealtimeSync(orgId, tables, load)
